...
public class SqlServerDatabaseClient : Db.Domain.DatabaseClient, IDisposable
{
	...
	public override Task<object[]> Query(Domain.DatabaseQuery query)
	{
		...
		while (reader.Read())
		{
			var rowAsDictionary = columnNames
				.ToDictionary(
					...
					columnName => GetValue(reader, columnName));
		...
		return Task.FromResult(BuildQueryResult(rows, query));
	}

	object[] BuildQueryResult(IEnumerable<object> rows, Domain.DatabaseQuery query)
	{
