...
public class DatabaseQuery
{
	...
	=> new DatabaseQuery(new[] { $"%{tableName}%", $"%{columnName}%" }, DatabaseQueryType.TablesAndColumnsBrowsing);

	public static DatabaseQuery ThatBrowsesAllTables
	=> new DatabaseQuery(new[] { "%" }, DatabaseQueryType.TablesAndColumnsBrowsing);

	public static DatabaseQuery ThatBrowsesRoutinesViewsAndTriggersNamesContaining(string partialName)
	=> new DatabaseQuery(new[] { $"%{partialName}%" }, DatabaseQueryType.RoutinesViewsAndTriggersBrowsing);

	public static DatabaseQuery ThatBrowsesRoutinesViewsAndTriggersExactlyNamed(string partialName)
	=> new DatabaseQuery(new[] { partialName }, DatabaseQueryType.RoutinesViewsAndTriggersBrowsing);

	public static DatabaseQuery ThatBrowsesRoutinesViewsAndTriggersNamesAndBodiesContaining(string partialName, string partialBody)
	=> new DatabaseQuery(new[] { $"%{partialName}%", $"%{partialBody}%" }, DatabaseQueryType.RoutinesViewsAndTriggersBrowsing);
